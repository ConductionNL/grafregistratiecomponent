# grafregistratiecomponent chart

The Helm chart installs grafregistratiecomponent and by default the following dependencies using subcharts:
...
```bash
helm repo add grafregistratiecomponent https://raw.githubusercontent.com/ConductionNL/grafregistratiecomponent/master/api/helm/
helm repo update
```
...
```bash
helm install my-grafregistratiecomponent ConductionNL/grafregistratiecomponent --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `grafregistratiecomponent` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `grafregistratiecomponent` |
